...
Check "Whole_weight" and "Diameter" is normally distributed:

```{r}
shapiro.test(all_data$Whole_weight)
```
```{r}
shapiro.test(all_data$Diameter)
```

In both cases p-value < 0.05. It means that out values is not normally distributed. 
Let's check this by plotting "Whole_weight" versus "Diameter" (Picture 12).
...
There is exponential relationship between variables. We can see that the non-linear model will better describe our data. 
Because of non-normal distribution of values we can't use Pearson's correlation, we must use Spearman's correlation. 
Sperman's correlation coefficient is 0.968. This indicates a strong relationship between the variables

```{r}
cor.test(all_data$Diameter, all_data$Whole_weight, method="spearman")
```
