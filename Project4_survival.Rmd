...
```{r}
data_survival <- data_survival %>% mutate(age_groups = ifelse(age >= m, "Group_1", "Group_2"))
...
The p-value is greater than 0.05 too.
There are no significant differences in survival by the all groups.
...
There is no significant factors which affect to survival. But age (less then 56 years), having no residual diseases and second type of treatment may affect to patient survival.
